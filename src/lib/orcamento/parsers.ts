...
export function parseTXT(txtContent: string): ParsedItemRow[] {
  ...
  for (const line of lines) {
    ...
    if (cols.length >= 2) {
      ...
      if (cols.length >= 5 && !isNaN(parseFloat(cols[1])) && !isNaN(parseFloat(cols[2]))) {
        const dim1 = parseFloat(cols[1]) || 0;
        const dim2 = parseFloat(cols[2]) || 0;
        ...
        code = cols[0] || '';
        desc = cols[1] || '';
        qty = parseFloat(cols[2]?.replace(',', '.')) || 1;
...
export function parseCSV(csvContent: string): ParsedItemRow[] {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    const qty = parseFloat(qtyRaw.replace(',', '.')) || 1;
...
export function parseJSON(jsonContent: string): ParsedItemRow[] {
  ...
  return array.map((it: any) => ({
    code: it.code || it.Referencia || it.codigo || '',
    description: it.description || it.Descricao || it.descricao || '',
    quantity: parseFloat(it.quantity || it.QtdTotal || it.quantidade || '1') || 1,
    unit: it.unit || it.Unidade || it.unidade || 'UN',
  }));
}
